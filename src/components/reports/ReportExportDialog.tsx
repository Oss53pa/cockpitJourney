...
export function ReportExportDialog({
  ...
  // Live preview (HTML render of the report with the current options). null =
  // we're showing the options form; a string = we're showing the preview pane.
  ...
  // Order sections per DEFAULT_SECTIONS so the doc stays canonical, and resolve
  // the project filter. Shared by export AND preview so they stay in sync.
  const buildOptions = () => ({
    sections: DEFAULT_SECTIONS.filter((s) => sections.has(s)),
    projectIds: projectFilter.size === 0 ? undefined : Array.from(projectFilter),
  });
  ...
  const handleExport = async () => {
    ...
    setBusy(true);
    try {
      await exportReport(format, { report, tasks, users, projectNames, options: buildOptions() });
      pushToast({
        kind: 'success',
  ...
  const handlePreview = async () => {
    ...
    setPreviewBusy(true);
    try {
      const html = await previewReportHtml({
        report,
        tasks,
        users,
        projectNames,
      ...
      const msg = err instanceof Error ? err.message : String(err);
      pushToast({ kind: 'error', title: 'Aperçu indisponible', body: msg });
    } finally {
      ...
      footer={
        <>
          ...
          <button
            ...
            disabled={previewBusy || busy || sections.size === 0}
          ...
          >
            {previewBusy ? (
              ...
              <>
                ...
                {previewHtml !== null ? 'Rafraîchir l’aperçu' : 'Aperçu'}
              </>
            )}
          </button>
          <button
      ...
      {previewHtml !== null ? (
        <div className="space-y-2">
          <p className="text-2xs text-atlas-fg-3 leading-relaxed">
            Aperçu du contenu (rendu Web). La mise en page PDF / PowerPoint peut différer — le contenu et les
            sections sélectionnées sont identiques.
          </p>
          <iframe
            title="Aperçu du rapport"
            srcDoc={previewHtml}
            sandbox="allow-same-origin"
            className="w-full h-[62vh] rounded-xl border border-atlas-line bg-white"
          />
        </div>
      ) : (
